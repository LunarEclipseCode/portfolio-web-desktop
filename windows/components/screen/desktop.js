...
export class Desktop extends Component {
  constructor() {
    ...
    this.allAppsRef = React.createRef();
  }
  ...
  handleClickOutside = (event) => {
    if (this.allAppsRef.current && !this.allAppsRef.current.contains(event.target)) {
      this.setState({ allAppsView: false }); // Close AllApplications if clicked outside
    }
  };
  ...
  renderWindows = () => {
    ...
    apps.forEach((app, index) => {
      if (this.state.closed_windows[app.id] === false) {
        ...
        // Show the application list above other applications
        windowsJsx.push(
          <div key={index} style={{ zIndex: this.state.allAppsView ? 0 : 1 }}>
            {" "}
            <Window {...props} />
          </div>
        );
  ...
  renderNameBar = () => {
    ...
    return (
      <div className="absolute rounded-md top-1/2 left-1/2 text-center text-white font-light text-sm bg-ub-cool-grey transform -translate-y-1/2 -translate-x-1/2 sm:w-96 w-3/4 z-50">
        ...
        <div className="flex">
          <div onClick={addFolder} className="w-1/2 px-4 py-2 border border-gray-900 border-opacity-50 border-r-0 hover:bg-ub-warm-grey hover:bg-opacity-10 hover:border-opacity-50">
            Create
          </div>
          <div onClick={removeCard} className="w-1/2 px-4 py-2 border border-gray-900 border-opacity-50 hover:bg-ub-warm-grey hover:bg-opacity-10 hover:border-opacity-50">
            Cancel
          </div>
        </div>
      </div>
  ...
  render() {
    return (
      <div className={"pb-10 h-full w-full flex flex-col items-end justify-start content-start flex-wrap pt-8 bg-transparent relative overflow-hidden overscroll-none window-parent"}>
        <style>
          {`
        ...
        {this.state.allAppsView ? (
          <div ref={this.allAppsRef} style={{ zIndex: 100 }}>
            {" "}
            {/* Wrapper with ref */}
            <AllApplications
              apps={apps}
              ...
              openApp={this.openApp}
              onClose={() => this.setState({ allAppsView: false })} // Pass onClose prop
            />
          </div>
        ) : null}
      </div>
