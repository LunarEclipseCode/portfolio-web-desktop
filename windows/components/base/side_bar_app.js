    ...
    render() {
        return (
            <div
                ...
                className={(this.props.isClose[this.id] === false && this.props.isFocus[this.id] ? "bg-white bg-opacity-10 " : "") + " w-auto p-2 pt-1.5 outline-none relative transition hover:bg-white hover:bg-opacity-10 rounded m-1"}
                ...
                <img className={(this.state.scaleImage ? " scale " : "") + " scalable-app-icon w-7 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"} src={this.props.icon} alt="" />
                {
                    (
                        this.props.isClose[this.id] === false
                            ? <div className=" w-1 h-1 absolute left-5 top-9 bg-gray-300 rounded-sm"></div>
                            : null
                    )
                }
                <div
