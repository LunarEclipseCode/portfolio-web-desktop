...
export class Window extends Component {
  ...
  restoreWindow = () => {
    ...
    setTimeout(() => {
      ...
      // No need to show the sidebar after restoring the window
      // this.checkOverlap();  // Sidebar visibility no longer depends on window size
    }, 300);
  };
  ...
  maximizeWindow = () => {
    if (this.state.maximized) {
      ...
      this.setState({ maximized: true, height: 91.2, width: 100.2 });
      // Remove or modify this line to keep the sidebar visible
      // this.props.hideSideBar(this.id, true); 
    }
  };  
    ...
    return (
      ...
      >
        <div
          ...
          className={this.state.cursorType +
            " " +
        ...
        >
          ...
          {this.id === "settings" ? (
            ...
            <WindowMainScreen screen={this.props.screen} title={this.props.title} addFolder={this.props.id === "terminal" ? this.props.addFolder : null} openApp={this.props.openApp} />
          )}
        </div>
      </Draggable>
    );
...
export function WindowTopBar(props) {
  return (
    <div className={" relative bg-ub-window-title border-t-2 border-white border-opacity-5 py-1.5 px-3 text-white w-full select-none "}>
...
export function WindowEditButtons(props) {
  return (
    <div className="absolute select-none right-0 top-0 mt-1 mr-1 flex justify-center items-center">
      <span className="mx-1.5 bg-white bg-opacity-0 hover:bg-opacity-10 rounded-full flex justify-center mt-0.5 mb-0.5 h-6 w-6 items-center" onClick={props.minimize}>
        <img src="./themes/Yaru/window/window-minimize-symbolic.svg" alt="ubuntu window minimize" className="h-5 w-5 inline" />
      </span>
      {props.isMaximised ? (
        <span className="mx-2 bg-white bg-opacity-0 hover:bg-opacity-10 rounded-full flex justify-center mt-0.5 mb-0.5 h-6 w-6 items-center" onClick={props.maximize}>
          <img src="./themes/Yaru/window/window-restore-symbolic.svg" alt="ubuntu window restore" className="h-5 w-5 inline" />
        </span>
      ) : (
        <span className="mx-2 bg-white bg-opacity-0 hover:bg-opacity-10 rounded-full flex justify-center mt-0.5 mb-0.5 h-6 w-6 items-center" onClick={props.maximize}>
          <img src="./themes/Yaru/window/window-maximize-symbolic.svg" alt="ubuntu window maximize" className="h-5 w-5 inline" />
        </span>
      )}
      <button tabIndex="-1" id={`close-${props.id}`} className="mx-1.5 focus:outline-none cursor-default bg-red-600 bg-opacity-0 hover:bg-opacity-100 rounded-full flex justify-center mt-0.5 mb-0.5 h-6 w-6 items-center" onClick={props.close}>
        <img src="./themes/Yaru/window/window-close-symbolic.svg" alt="ubuntu window close" className="h-5 w-5 inline" />
      </button>
    </div>
  );
}
...
export class WindowMainScreen extends Component {
  ...
  render() {
    return (
      <div className={"w-full flex-grow z-20 max-h-full overflow-y-auto windowMainScreen" + (this.state.setDarkBg ? " bg-ub-drk-abrgn " : " bg-ub-cool-grey")}>{this.props.addFolder ? displayTerminal(this.props.addFolder, this.props.openApp) : this.props.screen()}</div>
    );
  }
}
