...
const Window = (props: WindowProps) => {
  ...
  useEffect(() => {
    setState((prevState) => ({
      ...prevState,
      width: Math.min(winWidth, prevState.width),
      height: Math.min(winHeight, prevState.height),
      x: Math.max(
        winWidth + (winWidth - initWidth) / 2 + (props.x || 0),
        Math.min(prevState.x, winWidth - prevState.width - minMarginX)
      ),
      y: Math.max(
        minMarginY,
        Math.min(prevState.y, winHeight - prevState.height - minMarginY)
      )
    }));
  }, [winWidth, winHeight]);
