...
export class Algo extends Component {
  ...
  getSpeedDelay = () => {
    ...
    const adjustedSpeed =
      this.state.speed <= 75
        ? (this.state.speed / 75) * 90
        : 90 + ((this.state.speed - 75) / 25) * 10;
  ...
  renderArray = () => {
    ...
    const barNumber = 100 / barCount;
    const barInterval = totalSpacing / barCount; // Calculating as a percentage
        ...
        {array.map((value, index) => (
          <div
            ...
            style={{
              ...
              width: `calc(${barNumber}% - ${barInterval}px)`, // Adjust width based on spacing
  ...
  render() {
    const { isSorting, sorted, speed, selectedAlgorithm, comparisons, accesses, writes } =
      this.state;
    const runButtonLabel = !isSorting && !sorted ? "Run" : "";
    const isRunButtonDisabled =
      isSorting ||
      (!isSorting && sorted) ||
      (!isSorting && !sorted && !selectedAlgorithm);

    return (
      <div
        className="w-full h-full flex flex-col bg-gray-200 dark:bg-gray-800 text-black dark:text-white select-none"
        ref={this.containerRef}
      >
        <div
          className="w-full bg-gray-100 dark:bg-gray-700 text-sm p-1.5 flex flex-wrap items-center justify-between"
          ref={this.topBarRef}
        >
          <div className="flex items-center space-x-2">
            <button
              onClick={this.generateRandomArray}
              className="border border-gray-400 dark:border-gray-600 bg-gray-300 dark:bg-gray-600 px-3 py-1 rounded hover:bg-gray-400 dark:hover:bg-gray-500 focus:outline-none"
            >
              Generate
            </button>
            <select
              ...
              className={`border border-gray-400 dark:border-gray-600 bg-gray-300 dark:bg-gray-600 px-3 py-1 rounded focus:outline-none 
  ${isSorting || sorted ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-400 dark:hover:bg-gray-500"}`}
            >
              <option className="bg-white dark:bg-gray-700">Bubble Sort</option>
              <option className="bg-white dark:bg-gray-700">Quick Sort</option>
              <option className="bg-white dark:bg-gray-700">Merge Sort</option>
              <option className="bg-white dark:bg-gray-700">Insertion Sort</option>
              <option className="bg-white dark:bg-gray-700">Selection Sort</option>
              <option className="bg-white dark:bg-gray-700">Heap Sort</option>
              <option className="bg-white dark:bg-gray-700">Cocktail Sort</option>
              <option className="bg-white dark:bg-gray-700">Pancake Sort</option>
              <option className="bg-white dark:bg-gray-700">Comb Sort</option>
              <option className="bg-white dark:bg-gray-700">Shell Sort</option>
              <option className="bg-white dark:bg-gray-700">Bogo Sort</option>
            </select>
            <button
              onClick={this.handleRun}
              disabled={isRunButtonDisabled}
              className={`border border-gray-400 dark:border-gray-600 bg-gray-300 dark:bg-gray-600 px-3 py-1 rounded focus:outline-none ${isRunButtonDisabled ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-400 dark:hover:bg-gray-500"}`}
            >
              {isSorting ? (
          ...
          <div className="flex items-center space-x-2 mt-1 sm:mt-0">
            <label htmlFor="speedRange" className="text-black dark:text-white">
              Speed:
            </label>
            <input
              id="speedRange"
        ...
        <div className="flex-grow flex justify-center items-center bg-gray-300 dark:bg-gray-900 overflow-hidden relative">
          {this.renderArray()}
        </div>
        <div className="w-full bg-gray-100 dark:bg-gray-700 text-sm p-1 flex justify-around items-center">
