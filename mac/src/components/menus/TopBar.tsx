...
const TopBar = (props: TopBarProps) => {
  ...
  const [audio, audioState, controls, audioRef] = useAudio({
    src: music[currentMusicIndex].audio
  });
  ...
  useEffect(() => {
    const handleEnded = () => {
      ...
      setIsInitialPlay(true); // Ensure the next song plays automatically
      setPlaying(true); // Update playing state to true when next song starts
    };
  ...
  useEffect(() => {
    if (isInitialPlay) {
      audio.src = music[currentMusicIndex].audio;
      audio.play();
      setPlaying(true); // Ensure playing state is true when initial play starts
    } else {
      audio.src = music[currentMusicIndex].audio;
    }
  }, [currentMusicIndex, isInitialPlay]);
  ...
  const handlePrevious = () => {
    setCurrentMusicIndex((prevIndex) => (prevIndex - 1 + music.length) % music.length);
    setIsInitialPlay(true);
    setPlaying(true);
  };
    ...
    >
      ...
      <div className="hstack flex-row justify-end space-x-2">
        ...
        {state.showControlCenter && (
          <ControlCenterMenu
            playing={playing} // Use the new playing state
            toggleAudio={handleToggleAudio}
            nextSong={handleNext} // Add next song handler
            previousSong={handlePrevious} // Add previous song handler
            setVolume={setAudioVolume}
            setBrightness={setSiteBrightness}
            toggleControlCenter={toggleControlCenter}
            btnRef={controlCenterBtnRef}
            currentMusicIndex={currentMusicIndex}
            audioRef={audioRef} // Pass audioRef to ControlCenterMenu
          />
        )}
