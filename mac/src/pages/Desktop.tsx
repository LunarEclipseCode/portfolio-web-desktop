...
export default function Desktop(props: MacActions) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // Adjust the app sizes based on the new window size
      apps.forEach((app) => {
        const appElement = document.querySelector(`#window-${app.id}`) as HTMLElement;
        if (appElement) {
          const newWidth = Math.min(window.innerWidth, app.width);
          const newHeight = Math.min(window.innerHeight, app.height);
